...
import type { DiscordUser, AuthenticatedRequest, AppConfig } from '../../types/server.ts';
...
router.get(
  ...
  (req: Request, _res: Response, next: NextFunction) => {
    log.info('OAuth callback received', { query: req.query, sessionId: (req as any).sessionID });
    next();
  },
  ...
  (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) {
      ...
      return (res as any).redirect('/auth/error');
    }
    ...
    try {
      ...
      (req as any).login(req.user, (loginErr) => {
        if (loginErr) {
        ...
        req.session.save((err) => {
          ...
          log.debug(`Session ID: ${(req as any).sessionID}, Session data:`, {
            userId: user.id,
          ...
          (res as any).redirect(`${baseUrl}/`);
        });
      });
    } catch (error) {
      const err = error as Error;
      log.error(`Error in OAuth callback handler: ${err.message}`);
      (res as any).redirect('/auth/error');
    }
  }
);
...
router.get('/logout', (req: AuthenticatedRequest, res: Response): void => {
  ...
  (req as any).logout((err) => {
    if (err) {
      ...
      (res as any).status(500).json({ error: 'Logout failed' });
      return;
    }
...
router.get('/me', (req: AuthenticatedRequest, res: Response): void => {
  if (!(req as any).isAuthenticated || !(req as any).isAuthenticated()) {
    (res as any).status(401).json({ error: 'Not authenticated' });
    return;
  }
...
router.get('/check', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  log.debug(
    `Auth check requested - Session ID: ${(req as any).sessionID}, Authenticated: ${(req as any).isAuthenticated ? (req as any).isAuthenticated() : false}`
  );

  if (!(req as any).isAuthenticated || !(req as any).isAuthenticated()) {
    log.debug('Auth check: Not authenticated');
    (res as any).status(401).json({ authenticated: false });
    return;
  }
  ...
  try {
    ...
    (res as any).status(500).json({ error: 'Error verifying access' });
  }
});

// Error page with detailed error info
router.get('/error', (req: Request, res: Response): void => {
  const errorMessage = ((req as any).query['error'] as string) || 'Unknown error occurred';
  const errorDetails = ((req as any).query['details'] as string) || '';
...
router.get('/debug', (req: AuthenticatedRequest, res: Response) => {
  (res as any).json({
    authenticated: (req as any).isAuthenticated ? (req as any).isAuthenticated() : false,
    user: req.user || null,
    session: {
