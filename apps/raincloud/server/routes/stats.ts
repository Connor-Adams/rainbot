...
import { statsEmitter } from '../../utils/statistics.ts';
...
function parseDateRange(req: Request): DateRange {
  const startDate = parseValidDate((req as any).query['startDate'] as string | undefined);
  const endDate = parseValidDate((req as any).query['endDate'] as string | undefined);
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      (res as any).status(status).json({ error: err.message });
    }
  }
);
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const limit = Math.min(parseInt((req as any).query['limit'] as string) || 25, 100);
      const filters: WhereFilters = {
        guildId: (req as any).query['guildId'] as string | undefined,
        userId: (req as any).query['userId'] as string | undefined,
        source: (req as any).query['source'] as string | undefined,
        startDate: parseValidDate((req as any).query['startDate'] as string | undefined),
        endDate: parseValidDate((req as any).query['endDate'] as string | undefined),
      };
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const filters = {
        guildId: (req as any).query['guildId'] as string | undefined,
        userId: (req as any).query['userId'] as string | undefined,
        sourceType: (req as any).query['sourceType'] as string | undefined,
        isSoundboard:
          (req as any).query['isSoundboard'] !== undefined
            ? (req as any).query['isSoundboard'] === 'true'
            : undefined,
        startDate: parseValidDate((req as any).query['startDate'] as string | undefined),
        endDate: parseValidDate((req as any).query['endDate'] as string | undefined),
      };
...
export async function getUserSoundsHandler(req: Request, res: Response): Promise<void> {
  try {
    const userId = ((req as any).query['userId'] as string) || null;
    if (!userId) {
      (res as any).status(400).json({ error: 'Missing required query parameter: userId' });
      return;
    }

    const limit = Math.min(parseInt((req as any).query['limit'] as string) || 50, 200);
    const guildId = ((req as any).query['guildId'] as string) || undefined;
    const startDate = parseValidDate((req as any).query['startDate'] as string | undefined);
    const endDate = parseValidDate((req as any).query['endDate'] as string | undefined);
    ...
    (res as any).json({ sounds: result?.rows || [] });
  } catch (error) {
    const err = error as Error;
    const status = err.message.includes('Invalid date') ? 400 : 500;
    (res as any).status(status).json({ error: err.message });
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const granularity = ((req as any).query['granularity'] as string) || 'day'; // hour, day, week, month
      const filters = {
        guildId: (req as any).query['guildId'] as string | undefined,
        startDate: parseValidDate((req as any).query['startDate'] as string | undefined),
        endDate: parseValidDate((req as any).query['endDate'] as string | undefined),
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const filters: WhereFilters = {
        guildId: (req as any).query['guildId'] as string | undefined,
        operationType: (req as any).query['operationType'] as string | undefined,
        startDate: parseValidDate((req as any).query['startDate'] as string | undefined),
        endDate: parseValidDate((req as any).query['endDate'] as string | undefined),
      };
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = ((req as any).query['userId'] as string) || null; // null means all users
      const guildId = ((req as any).query['guildId'] as string) || null;
      const limit = Math.min(parseInt((req as any).query['limit'] as string) || 25, 100);
      const startDate = parseValidDate((req as any).query['startDate'] as string | undefined);
      const endDate = parseValidDate((req as any).query['endDate'] as string | undefined);

      const listeningHistory = await import('../../utils/listeningHistory');
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const filters = {
        guildId: (req as any).query['guildId'] as string | undefined,
        commandName: (req as any).query['commandName'] as string | undefined,
        startDate: parseValidDate((req as any).query['startDate'] as string | undefined),
        endDate: parseValidDate((req as any).query['endDate'] as string | undefined),
      };
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const filters = {
        guildId: (req as any).query['guildId'] as string | undefined,
        interactionType: (req as any).query['interactionType'] as string | undefined,
        startDate: parseValidDate((req as any).query['startDate'] as string | undefined),
        endDate: parseValidDate((req as any).query['endDate'] as string | undefined),
      };
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const filters = {
        guildId: (req as any).query['guildId'] as string | undefined,
        stateType: (req as any).query['stateType'] as string | undefined,
        startDate: parseValidDate((req as any).query['startDate'] as string | undefined),
        endDate: parseValidDate((req as any).query['endDate'] as string | undefined),
      };
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const limit = Math.min(parseInt((req as any).query['limit'] as string) || 25, 100);
      const filters = {
        eventType: (req as any).query['eventType'] as string | undefined,
        startDate: parseValidDate((req as any).query['startDate'] as string | undefined),
        endDate: parseValidDate((req as any).query['endDate'] as string | undefined),
      };
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const filters = {
        endpoint: (req as any).query['endpoint'] as string | undefined,
        startDate: parseValidDate((req as any).query['startDate'] as string | undefined),
        endDate: parseValidDate((req as any).query['endDate'] as string | undefined),
      };
...
router.get('/stream', statsRateLimiter, requireAuth, (req: Request, res: Response) => {
  ...
  (statsEmitter as any).on('batchInserted', onBatch);
  (statsEmitter as any).on('flushed', onFlush);

  // Immediately send a ping so client knows connection is live
  send('connected', { ts: new Date().toISOString() });

  (req as any).on('close', () => {
    (statsEmitter as any).off('batchInserted', onBatch);
    (statsEmitter as any).off('flushed', onFlush);
    try {
      (res as any).end();
    } catch {
      // no-op
...
router.get('/check-tables', statsRateLimiter, requireAuth, async (_req: Request, res: Response) => {
  try {
    ...
    (res as any).json({ tables: counts, ts: new Date().toISOString() });
  } catch (error) {
    const err = error as Error;
    (res as any).status(500).json({ error: err.message });
  }
