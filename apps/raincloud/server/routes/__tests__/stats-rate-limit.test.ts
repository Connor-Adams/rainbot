...
Deno.test('enforces rate limit after excessive requests', async () => {
...
Deno.test('includes rate limit headers in response', async () => {
  const response = await request(app).get('/api/stats/summary');
...
Deno.test('applies rate limiting to all stats endpoints', async () => {
  const endpoints = [
    '/api/stats/summary',
    '/api/stats/commands',
    '/api/stats/sounds',
