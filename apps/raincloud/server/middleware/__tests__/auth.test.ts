...
Deno.test('returns 401 when user is not authenticated', async () => {
  ...
  Deno.test('returns 401 when isAuthenticated is not defined', async () => {
  ...
  Deno.test('returns 401 when user is undefined', async () => {
  ...
  Deno.test('returns 401 when user id is missing', async () => {
  ...
  Deno.test('allows access when requiredRoleId is not configured', async () => {
  ...
  Deno.test('uses cached verification when valid', async () => {
    const now = Date.now();
    mockReq.session = {
      hasAccess: true,
  ...
  Deno.test('verifies role when cache is expired', async () => {
    const now = Date.now();
    mockReq.session = {
      hasAccess: true,
  ...
  Deno.test('verifies role when no cache exists', async () => {
  ...
  Deno.test('grants access when user has required role', async () => {
  ...
  Deno.test('denies access when user lacks required role', async () => {
  ...
  Deno.test('updates session cache after successful verification', async () => {
    mockVerifyUserRole.mockResolvedValueOnce(true);
    mockReq.session = {} as any;

    await requireAuth(mockReq as AuthenticatedRequest, mockRes as Response, mockNext);

    assertEquals(mockReq.session?.hasAccess, true);
    assert(mockReq.session?.lastVerified > 0);
    expect(mockNext).toHaveBeenCalled();
  });

  Deno.test('updates session cache after failed verification', async () => {
    mockVerifyUserRole.mockResolvedValueOnce(false);
    mockReq.session = {} as any;

    await requireAuth(mockReq as AuthenticatedRequest, mockRes as Response, mockNext);

    assertEquals(mockReq.session?.hasAccess, false);
    assert(mockReq.session?.lastVerified > 0);
    expect(mockNext).not.toHaveBeenCalled();
  });

  Deno.test('handles verification errors gracefully', async () => {
  ...
  Deno.test('works when session is undefined', async () => {
