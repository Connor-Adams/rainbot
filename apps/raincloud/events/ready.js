...
const { deployCommands } = require('../utils/deployCommands.ts');
const { loadConfig } = require('../utils/config.ts');
...
module.exports = {
  ...
  async execute(client) {
    ...
    try {
      const { restoreAllQueueSnapshots, startAutoSave } = require('../utils/voiceManager.ts');
      const { waitForSchema, getPool } = require('../utils/database.ts');
