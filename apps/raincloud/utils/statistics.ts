...
function startBatchProcessor(): void {
  ...
  batchTimer = (setInterval as any)(async () => {
    await flushBatches();
  }, BATCH_INTERVAL);
...
async function insertBatch(type: BufferType, table: string, events: unknown[]): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      (statsEmitter as any).emit('batchInserted', {
        type,
        table,
        count: events.length,
...
export async function flushAll(): Promise<void> {
  ...
  await flushBatches();
  if (batchTimer) {
    (clearInterval as any)(batchTimer);
    batchTimer = null;
  }
  try {
    (statsEmitter as any).emit('flushed', { ts: new Date().toISOString() });
  } catch {
    /* ignore */
  }
}
...
export async function getOverallStats(): Promise<{
  ...
  try {
    ...
    const tracksResult = await query('SELECT COUNT(*) as count FROM sound_stats');
    const totalTracks = parseInt(tracksResult?.rows?.[0]?.count || '0');

    // Get unique users
    const usersResult = await query('SELECT COUNT(DISTINCT user_id) as count FROM sound_stats');
    const totalUsers = parseInt(usersResult?.rows?.[0]?.count || '0');

    // Get unique guilds
    const guildsResult = await query('SELECT COUNT(DISTINCT guild_id) as count FROM sound_stats');
    const totalGuilds = parseInt(guildsResult?.rows?.[0]?.count || '0');

    // Get total plays (same as total tracks for now)
    const totalPlays = totalTracks;
    ...
    log.error('Failed to get overall stats:', error as Record<string, unknown>);
    return {
      totalTracks: 0,
      totalUsers: 0,
      totalGuilds: 0,
...
export async function getGuildStats(guildId: string): Promise<{
  ...
  try {
    // Get tracks played in this guild
    const tracksResult = await query(
      'SELECT COUNT(*) as count FROM sound_stats WHERE guild_id = $1',
      [guildId]
    );
    const tracksPlayed = parseInt(tracksResult?.rows?.[0]?.count || '0');

    // Get unique users in this guild
    const usersResult = await query(
      'SELECT COUNT(DISTINCT user_id) as count FROM sound_stats WHERE guild_id = $1',
      [guildId]
    );
    const usersActive = parseInt(usersResult?.rows?.[0]?.count || '0');

    // Get total time played (sum of durations)
    const timeResult = await query(
      'SELECT COALESCE(SUM(duration), 0) as total FROM sound_stats WHERE guild_id = $1 AND duration IS NOT NULL',
      [guildId]
    );
    const totalTime = parseInt(timeResult?.rows?.[0]?.total || '0');
    ...
    log.error('Failed to get guild stats:', error as Record<string, unknown>);
    return {
      guildId,
      tracksPlayed: 0,
      usersActive: 0,
 ...
 */
export async function getTopTracks(limit: number = 10): Promise<
  Array<{
    soundName: string;
    playCount: number;
    lastPlayed: Date;
 ...
 */
export async function getTopCommands(limit: number = 10): Promise<
  Array<{
    commandName: string;
    usageCount: number;
    successRate: number;
