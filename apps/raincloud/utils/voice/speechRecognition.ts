...
class GoogleSTTProvider implements STTProvider {
  ...
  async recognize(audioBuffer: Uint8Array, languageCode: string): Promise<SpeechRecognitionResult> {
    try {
      const audio = {
        content: (audioBuffer as any).toString('base64'),
      };
...
class OpenAISTTProvider implements STTProvider {
  ...
  async recognize(audioBuffer: Uint8Array, languageCode: string): Promise<SpeechRecognitionResult> {
    try {
      ...
      if (actualDuration < minDuration) {
        ...
        const paddedBuffer = (Buffer as any).alloc(minBytes);
        (audioBuffer as any).copy(paddedBuffer, 0);
  ...
  private async createFile(blob: Blob, filename: string): Promise<File> {
    ...
    const buffer = (Buffer as any).from(await blob.arrayBuffer());
    return {
      name: filename,
      type: 'audio/wav',
  ...
  private pcmToWav(pcmBuffer: Uint8Array, sampleRate: number, channels: number): Uint8Array {
    ...
    const wavBuffer = (Buffer as any).alloc(headerSize + dataSize);
    ...
    (pcmBuffer as any).copy(wavBuffer, 44);
...
export class SpeechRecognitionManager {
  ...
  async processDiscordAudio(audioChunks: Uint8Array[]): Promise<SpeechRecognitionResult> {
    ...
    const audioBuffer = (Buffer as any).concat(audioChunks);
    log.debug(
  ...
  private stereoToMono(stereoBuffer: Uint8Array): Uint8Array {
    ...
    const monoBuffer = (Buffer as any).alloc(samples * 2);
    ...
    for (let i = 0; i < samples; i++) {
      ...
      const leftSample = (stereoBuffer as any).readInt16LE(offset);
      const rightSample = (stereoBuffer as any).readInt16LE(offset + 2);
  ...
  detectSpeech(audioBuffer: Uint8Array, threshold: number = 500): boolean {
    ...
    for (let i = 0; i < samples; i++) {
      const sample = (audioBuffer as any).readInt16LE(i * 2);
