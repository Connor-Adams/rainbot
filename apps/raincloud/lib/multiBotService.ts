...
import { WorkerCoordinator } from './workerCoordinator.ts';
import { VoiceStateManager } from './voiceStateManager.ts';
import { ChannelResolver } from './channelResolver.ts';
...
export class MultiBotService {
  ...
  async getStatus(guildId: string): Promise<VoiceStatus | null> {
    try {
      ...
      const workerStatusesTyped = workerStatuses as Record<string, any>;
      const rainbotStatus = workerStatusesTyped['rainbot'] || {};
      ...
      return {
        ...
        workers: {
          ...
          pranjeet: {
            connected: workerStatusesTyped['pranjeet']?.connected || false,
            playing: workerStatusesTyped['pranjeet']?.playing || false,
          },
          hungerbot: {
            connected: workerStatusesTyped['hungerbot']?.connected || false,
            playing: workerStatusesTyped['hungerbot']?.playing || false,
            activePlayers: workerStatusesTyped['hungerbot']?.activePlayers || 0,
          },
        },
      };
    } catch (error) {
