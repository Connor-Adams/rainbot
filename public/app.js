...
function renderConnections() {
  if (connections.length === 0) {
    connectionsList.innerHTML =
      '<p class="empty-state text-gray-500 text-sm text-center py-8 px-6 flex flex-col items-center gap-2"><span class="text-2xl opacity-50">🔇</span>No active connections<br><small class="block mt-2 text-xs">Join a voice channel to get started</small></p>';
...
function updateNowPlayingCard(title, guildId) {
  ...
  const nowPlayingTrack =
    currentTrack ||
    currentQueue.find((t) => t.title === title) ||
  ...
  if (nowPlayingTrack) {
    ...
    if (nowPlayingTrack.isLocal) {
      ...
      nowPlayingTrack.url?.includes('youtube') ||
      nowPlayingTrack.url?.includes('youtu.be')
    ) {
...
function renderServers() {
  if (guilds.length === 0) {
    serversList.innerHTML =
      '<p class="empty-state text-gray-500 text-sm text-center py-8 px-6 flex flex-col items-center gap-2"><span class="text-2xl opacity-50">🏠</span>No servers available</p>';
    return;
  }

  serversList.innerHTML = guilds
    .map(
      (guild) => `
...
function renderSounds(filter = '') {
  const filtered = sounds.filter((s) => s.name.toLowerCase().includes(filter.toLowerCase()));
...
function renderQueue() {
  ...
  if (!queueData.nowPlaying && queueData.queue.length === 0) {
    queueList.innerHTML =
      '<p class="queue-empty text-center py-8 text-gray-500"><span class="block text-2xl mb-2 opacity-50">🎵</span>Queue is empty<br><small class="block mt-2 text-sm">Add tracks to start playing</small></p>';
    return;
  }
...
async function fetchStatus() {
  try {
    ...
    connectionsList.innerHTML =
      '<p class="empty-state text-gray-500 text-sm text-center py-8 px-6 flex flex-col items-center gap-2"><span class="text-2xl opacity-50">❌</span>Failed to load connections</p>';
    serversList.innerHTML =
      '<p class="empty-state text-gray-500 text-sm text-center py-8 px-6 flex flex-col items-center gap-2"><span class="text-2xl opacity-50">❌</span>Failed to load servers</p>';
  }
}
...
async function fetchSounds() {
  try {
    ...
    soundsGrid.innerHTML =
      '<p class="empty-state text-gray-500 text-sm text-center py-8 px-6 flex flex-col items-center gap-2"><span class="text-2xl opacity-50">❌</span>Failed to load sounds</p>';
  }
}
