...
async function playSoundboardOverlay(
  ...
  try {
    ...
    const ffmpegArgs = [
      ...
      '5',
      '-ss',
      playbackPosition.toString(), // Fast seek to approximate position
      '-i',
      musicStreamUrl, // Input 0: Music stream
      '-i',
      soundInput, // Input 1: Soundboard (piped from S3)
      '-filter_complex',
      ...
      'libopus',
      '-b:a',
      '128k', // Bitrate for better quality
      '-f',
      'opus',
      '-ar',
      '48000',
  ...
  if (source.startsWith('http://') || source.startsWith('https://')) {
    ...
    if (urlType === 'yt_playlist') {
      ...
      if (firstVideoId) {
        ...
        getStreamUrl(firstTrack.url).catch((err) =>
          log.debug(`Pre-fetch stream URL failed: ${err.message}`)
        );
      ...
      try {
        if (urlType === 'sp_track') {
          ...
          if (ytResults && ytResults.length > 0) {
            ...
            throw new Error(
              `Could not find YouTube equivalent for Spotify track: ${spotifyTrack.name}`
            );
          }
          ...
          log.info(
            `Found ${playlistTracks.length} tracks in Spotify playlist: ${spotifyPlaylist.name}`
          );
...
async function playWithSeek(state, track, seekSeconds, startPaused = false) {
  ...
  log.info(
    `Resumed playback of "${track.title}" at ${seekSeconds}s${startPaused ? ' (paused)' : ''}`
  );
}
