 ...
 */
export async function saveQueueSnapshot(guildId: string, state: VoiceState): Promise<void> {
 ...
 */
export async function saveAllQueueSnapshots(voiceStates: Map<string, VoiceState>): Promise<void> {
  const promises: Promise<void>[] = [];
  for (const [guildId, state] of voiceStates) {
    promises.push(
      saveQueueSnapshot(guildId, state).catch((e) =>
...
type JoinChannelFn = (channel: VoiceBasedChannel) => Promise<unknown>;
type PlayWithSeekFn = (
  state: VoiceState,
  track: Track,
  seekSeconds: number,
...
type PlayNextFn = (guildId: string) => Promise<Track | null>;
...
export async function restoreQueueSnapshot(
  ...
  try {
    ...
    const { getVoiceState } = await import('../voiceManager');
 ...
 */
export async function restoreAllQueueSnapshots(
  client: Client,
  joinChannel: JoinChannelFn,
  playWithSeek: PlayWithSeekFn,
  playNext: PlayNextFn
